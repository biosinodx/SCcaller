...
parser.add_argument("-a", "--analysis", type=str,required=True, help="{goldensnp, varcall, cutoff}")

parser.add_argument("-i", "--input", type=str,required=True, help="The input file, required. {goldensnp, varcall} mpileup file generated using 'samtools mpileup -C50 -Osf refgenome.fa input.bam > input.mpileup', samtools version 1.3 was tested good; {cutoff}: bedfile generated from {varcall}")
parser.add_argument("-o","--output",type=str,help="The output header, required for {godensnp, varcall}")
parser.add_argument("-s","--gss",type=str,help="Input golden standard snps - required for {varcall} output from {goldensnp} for bias estimation")
...
parser.add_argument("--RD",type=int,default=20,help="Golden heterogous call required read depths. Default: 20. Recommand: 10,15,20, depending on average read depth")
parser.add_argument("--snp",type=str,default='control',help="Known snp type input for golden heterogous call {dbsnp, control}. Default: control.")
parser.add_argument("--snpin",type=str,default='',help="Known snp input file for golden heterogous call. file type: bed (1-based) or vcf. NOTE: should be sorted in the same order as the mpileup file.")
...
### golden standard heterogeneity call read depth
gd_rd=args.RD
...
### snp input for golden heterogous call {dbsnp, control}
snptype=args.snp
#snptype='control'
...
### golden heterogenous site call for input bias estimates, using mpileup file and candiddate golden (dbsnp) or real golden (control) input
def golden_hetero(gh_mpinfile, gh_snpinfile, gh_outfile, gh_snptype='control', gh_readdepth=gd_rd):
	...
	while True:
		...
		if len(gh_mpline)==0:
			break
		while gh_snpline[0]=='#' and len(gh_snpline.split('\t'))!=mpu_ncol:
			gh_snpline=gh_snpin.readline()
			if len(gh_snpline)==0:
				break
		if len(gh_snpline)==0:
			break
		while gh_mpline.split('\t')[0] != gh_snpline.split('\t')[0] or len(gh_mpline.split('\t'))<mpu_ncol or len(gh_snpline.split('\t'))<mpu_ncol:
		...
		if len(gh_mpline)==0 or len(gh_snpline)==0:
			break
		while gh_mpline.split('\t')[1] != gh_snpline.split('\t')[1] or len(gh_mpline.split('\t'))<mpu_ncol or len(gh_snpline.split('\t'))<mpu_ncol:
		...
		gh_xrefcount=0
		gh_xaltcount=0
		# current golden standard snp allele read counts are based on all mapped reads without considering base quality
		for i in gh_x[6:]:
		...
		if gh_snptype=='dbsnp' and gh_xaltcount==0 and (gh_xrefcount + gh_xaltcount < gh_readdepth) :
			continue
...
### var_calling total, vc_mpileupfile - mpileupfile; vc_snpfile from formated golden hetero snp;
...
def main(analysis):
	if analysis=='goldensnp':
		golden_hetero(gh_mpinfile=args.input, gh_snpinfile=args.snpin, gh_outfile=args.output+'.goldensnp.bed', gh_snptype=args.snp, gh_readdepth=gd_rd)
	if analysis=='varcall':
